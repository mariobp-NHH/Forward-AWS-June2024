...
""" Arbeidkrav1 """
def ak1_t1(fakturpris, indirekte_kostnader, fortjeneste, konkurrent):
    fakturpris_sett=round(int(fakturpris)/20,1)
    intakskost_hele=int(fakturpris)+2000
    intakskost_sett=round(intakskost_hele/20,1)
    faste_hele=round(intakskost_hele*int(indirekte_kostnader)/100,1)
    faste_sett=round(faste_hele/20,1)
    selvkost_hele=round(intakskost_hele+faste_hele,1)
    selvkost_sett=round(selvkost_hele/20,1)
    fortjeneste_hele=round(selvkost_hele*int(fortjeneste)/100,1)
    fortjeneste_sett=round(fortjeneste_hele/20,1)
    salgspris_hele=round(selvkost_hele+fortjeneste_hele,1)
    salgspris_sett=round(salgspris_hele/20,1)
    mva_hele=round(salgspris_hele*0.25,1)
    mva_sett=round(mva_hele/20,1)
    salgspris_mva_hele=round(salgspris_hele+mva_hele,1)
    salgspris_mva_sett=round(salgspris_mva_hele/20,1)
    avanse=round(salgspris_hele-intakskost_hele,1)
    avanse_persent=round(avanse*100/intakskost_hele,1)
    pris_konkurrent=round(int(konkurrent)/1.25,1)
    ny_forjeneste=round(pris_konkurrent-selvkost_sett,1)
    ny_forjeneste_persent=round(ny_forjeneste*100/selvkost_sett,1)
    return(fakturpris_sett,intakskost_hele,intakskost_sett,faste_hele,
    ...
    ny_forjeneste_persent)

def ak1_t2(materiale,lonn,arbeid,ferdig):
    endring_arbeid=round(int(arbeid)-12000,1)
    endring_ferdig=round(int(ferdig)-68000,1)
    endring_arbeid_table=-1*endring_arbeid
    endring_ferdig_table=-1*endring_ferdig
    indirekte_material=round(int(materiale)*0.075,1)
    dekning_indirekte_material= round(indirekte_material-12100,1)
    indirekte_lonn=round(int(lonn)*0.6,1)
    dekning_indirekte_lonn= round(indirekte_lonn-46450,1)
    periodens_tilv_kal=round(int(materiale)+int(lonn)+indirekte_material+indirekte_lonn,1)
    periodens_tilv_virk=round(int(materiale)+int(lonn)+12100+46450,1)
    tilv_ferdig_kal=round(periodens_tilv_kal+endring_arbeid_table,1)
    tilv_ferdig_virk=round(periodens_tilv_virk+endring_arbeid_table,1)
    tilv_solgte_kal=round(tilv_ferdig_kal+endring_ferdig_table,1)
    tilv_solgte_virk=round(tilv_ferdig_virk+endring_ferdig_table,1)
    indirekte_adm_kal=round(tilv_solgte_kal*0.12,1)
    indirekte_adm_virk=39500
    dekning_indirekte_adm=round(indirekte_adm_kal-indirekte_adm_virk,1)
    selvkost_kal=round(tilv_solgte_kal+indirekte_adm_kal,1)
    selvkost_virk=round(tilv_solgte_virk+indirekte_adm_virk,1)
    produkt_resultat_kal=round(350000-selvkost_kal,1)
    dekning=round(dekning_indirekte_material+dekning_indirekte_lonn+dekning_indirekte_adm,1)
    produksjonresultat_kal=round(produkt_resultat_kal+dekning,1)
    produksjonresultat_virk=round(350000-selvkost_virk,1)
    return(endring_arbeid,endring_ferdig,endring_arbeid_table,
    ...
    produksjonresultat_virk)

def ak1_t3(kjop,produksjon,dm_240,dm_241):
    kjop_sat=round(int(kjop)*100/55000000,1)
    produksjon_sat=round(int(produksjon)/175000,1)
    tilvkost_a=round(55000000+67000000+int(kjop)+int(produksjon),1)
    adm_sat=round(6550000*100/tilvkost_a,1)
    selvkost_a=round(tilvkost_a+6550000,1)
    indirek_mat_240=round(int(dm_240)*kjop_sat/100,1)
    indirek_mat_241=round(int(dm_241)*kjop_sat/100,1)
    tilvkost_240=round(int(dm_240)+750000+indirek_mat_240+25000,1)
    tilvkost_241=round(int(dm_241)+490000+indirek_mat_241+16340,1)
    av_kjop_241=1200000*kjop_sat/100
    av_kjop_242=300000*kjop_sat/100
    av_kjop_243=2300000*kjop_sat/100
    av_kjop_sum=av_kjop_241+av_kjop_242+av_kjop_243
    av_kjop_dek=av_kjop_sum-400000
    av_produksjon_241=1083*produksjon_sat
    av_produksjon_242=392*produksjon_sat
    av_produksjon_243=2917*produksjon_sat
    av_produksjon_sum=av_produksjon_241+av_produksjon_242+av_produksjon_243
    av_produksjon_dek=av_produksjon_sum-75000
    til_perioden_241=1200000+650000+av_kjop_241+av_produksjon_241
    til_perioden_242=300000+235000+av_kjop_242+av_produksjon_242
    til_perioden_243=2300000+1750000+av_kjop_243+av_produksjon_243
    til_perioden_sum=3800000+2635000+av_kjop_sum+av_produksjon_sum
    til_ferd_241=til_perioden_241+tilvkost_241
    til_ferd_242=til_perioden_242
    til_ferd_243=til_perioden_243
    til_ferd_sum=til_perioden_sum+tilvkost_241
    til_ferd_virk=6910000+tilvkost_241
    behold_fv_242=-1*til_ferd_242
    behold_fv_sum=tilvkost_240+behold_fv_242
    til_solgte_sum=til_ferd_sum+behold_fv_sum
    til_solgte_virk=til_ferd_virk+behold_fv_sum
    indirekt_adm_240=tilvkost_240*adm_sat/100
    indirekt_adm_241=til_ferd_241*adm_sat/100
    indirekt_adm_243= til_ferd_243*adm_sat/100
    indirekt_adm_sum=til_solgte_sum*adm_sat/100
    indirekt_adm_dek=indirekt_adm_sum-390000
    selv_240=tilvkost_240+indirekt_adm_240
    selv_241=til_ferd_241+indirekt_adm_241
    selv_243=til_ferd_243+indirekt_adm_243
    selv_sum=til_solgte_sum+indirekt_adm_sum
    selv_virk=til_solgte_virk+390000
    resul_240=2000000-selv_240
    resul_241=4200000-selv_241
    resul_243=4800000-selv_243
    resul_sum=11000000-selv_sum
    sum_dek=av_produksjon_dek+av_kjop_dek+indirekt_adm_dek
    resultat_sum=resul_sum+sum_dek
    resultat_virk=11000000-selv_virk
    return(kjop_sat,produksjon_sat,tilvkost_a,adm_sat,selvkost_a,
    indirek_mat_240,indirek_mat_241,tilvkost_240,tilvkost_241,
    av_kjop_241,av_kjop_242,av_kjop_243,av_kjop_sum,av_kjop_dek,
    av_produksjon_241,av_produksjon_242,av_produksjon_243,
    av_produksjon_sum,av_produksjon_dek,
    til_perioden_241,til_perioden_242,til_perioden_243,til_perioden_sum,
    til_ferd_241,til_ferd_242,til_ferd_243,til_ferd_sum,til_ferd_virk,
    behold_fv_242,behold_fv_sum,
    til_solgte_sum,til_solgte_virk,
    indirekt_adm_240,indirekt_adm_241,indirekt_adm_243,indirekt_adm_sum,indirekt_adm_dek,
    selv_240,selv_241,selv_243,selv_sum,selv_virk,
    resul_240,resul_241,resul_243,resul_sum,sum_dek,resultat_sum,resultat_virk)

def ak1_t4(var_adm,enhet,faste_til,faste_adm):
    ts=round(int(enhet)*2.5,1)
    iv_admin=int(enhet)*int(var_adm)
    minkost=1792250+iv_admin
    kal_dek=2300500-minkost
    vir_dek=kal_dek+4500+22225
    vir_dek_vir=2300500-iv_admin-655000-1034775-75750
    fast_til_dek=int(faste_til)-201500
    fast_adm_dek=int(faste_adm)-184000
    fore=vir_dek-int(faste_til)-int(faste_adm)
    fast_avvik=fast_til_dek+fast_adm_dek
    produk_std=fore+fast_avvik
    produk_vir=vir_dek_vir-201500-184000
    ls_ts=400*ts
    one_two=ls_ts-1022000
    one_three=ls_ts-1034775
    return(ts,iv_admin,minkost,kal_dek,vir_dek,vir_dek_vir,
    fast_til_dek,fast_adm_dek,
    fore,fast_avvik,produk_std,produk_vir,
    ls_ts,one_two,one_three)
